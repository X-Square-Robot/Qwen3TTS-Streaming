...
if "tests" in sys.modules:
    ...
    if not getattr(_t, "__file__", None) or not str(
        getattr(_t, "__file__", "")
    ).startswith(_repo_str):
...
def _check_triton_client():
    try:
        ...
        logger.error(
            "tritonclient[grpc] not installed.  pip install tritonclient[grpc]"
        )
        sys.exit(1)
...
def _stream_tts(client, grpcclient, text: str, speaker: str, timeout: float = 60.0):
    req_dict = build_request_payload(
        text=text, task_type="custom_voice", speaker=speaker
    )
...
def mode_gen_triton(args):
    ...
    for sample in SAMPLES_GEN_TRITON:
        ...
        chunks, first_sec, total_sec, err = _stream_tts(
            client, text, speaker, timeout=args.timeout
        )
        if err:
            print(f"    ERROR: {err}")
            continue
        ...
        print(
            f"    -> {wav_path.name}  duration={duration:.2f}s  "
...
def _engine_synthesize(
    host: str, port: int, text: str, speaker: str = "Serena", timeout: float = 60.0
...
def mode_gen_engine(args):
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else _default_out("audio_samples", "reference")
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    print(DEPRECATION_NOTICE)
    ...
    for name, text in TEXTS_ENGINE.items():
        ...
        audio, elapsed, n_chunks = _engine_synthesize(
            host, port, text, speaker=args.speaker
        )
        if audio is not None:
            ...
            print(
                f"  -> {out_path.name}: {len(audio)} samples, {duration:.2f}s, "
...
def mode_gen_reference(args):
    ...
    for name, text in TEXTS_REFERENCE.items():
        ...
        print(
            f"  -> {out_path.name}: {len(wav)} samples, {duration:.2f}s, took {elapsed:.1f}s"
        )
...
def mode_compare_triton(args):
    ...
    logger.info(
        "Wrote proto.wav (official API, %d samples, %.2fs)",
        len(wav_proto),
        len(wav_proto) / sr,
    )
...
def mode_compare_full_chain(args):
    ...
    out_root = (
        Path(args.out_root) if args.out_root else _default_out("audio_compare", "runs")
    )
...
def mode_compare_3way(args):
    ...
    from generate_audio_compare import (
        run_ort_decode_loop,
        decode_codes_to_wav,
        OFFICIAL_ASSISTANT_FMT,
    )
    ...
    if args.triton_url:
        try:
            ...
            req_dict = build_variant_request_payload(
                ...
                text=args.text,
                language=args.language,
                speaker=args.speaker or "serena",
            ...
            if stream.audio is not None and stream.audio.size:
                ...
                logger.info(
                    "TRT (orchestrator): %d samples (%.2fs)",
                    len(wav_trt),
                    len(wav_trt) / sample_rate,
                )
    ...
    wav_manual, _ = decode_codes_to_wav(
        model, trim_at_eos(codes_manual, manual_eos), device
    )
    ...
    if wav_trt is not None:
        report_lines.append(
            f"  TRT:        {len(wav_trt)} samples ({len(wav_trt) / sr:.2f} s)"
        )
    report_lines += [
        "",
...
def mode_fused_onnx(args):
    ...
    logger.info(
        "Prefill embeds: %s, trailing: %d, codec_eos_id: %d",
    ...
    talker_past_kv = np.empty(
        (batch, num_layers * 2, num_kv_heads, 0, head_dim), dtype=np.float32
    )
    ...
    def build_feed(inp, pos, cache_pos_val, past_kv_np, c2w_kv_np, c2w_st, tc):
        ...
        feed = {
            ...
            "attention_bias": np.zeros(
                (batch, 1, cur_seq, past_len + cur_seq), dtype=np.float32
            ),
            ...
            "penalty": penalty,
            "cache_position": np.full(
                (batch, chunk_t), cache_pos_val, dtype=np.float32
            ),
    ...
    feed = build_feed(
        inp_np, pos_np, 0, talker_past_kv, c2w_past_kv, c2w_states_np, token_counts
    )
    ...
    text_add = (
        trailing[0].detach().cpu().float().numpy()
        if len(trailing) > 0
        else pad_embed.detach().cpu().float().numpy()
    )
    if text_add.ndim == 2:
        text_add = text_add.reshape(1, 1, -1)
    next_input = (codec_sum.astype(np.float64) + text_add.astype(np.float64)).astype(
        np.float32
    )

    logger.info(
        "Prefill done: codec_0=%d, wav_samples=%d",
        codec_0,
    ...
    for step in range(1, args.max_steps):
        ...
        wav_chunk = outs["wav"]
        token_counts = outs["updated_token_counts"].copy()
        talker_past_kv = np.concatenate(
            [talker_past_kv, outs["talker_new_kv"]], axis=3
        ).copy()
        ...
        if text_add.ndim == 2:
            text_add = text_add.reshape(1, 1, -1)
        next_input = (
            codec_sum.astype(np.float64) + text_add.astype(np.float64)
        ).astype(np.float32)
...
def mode_long_ab(args):
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else _default_out("audio_compare", f"listen_{args.case}")
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    print(DEPRECATION_NOTICE)
    ...
    if not args.skip_engine:
        ...
        if _check_server(host, port):
            ...
            manifest["engine"] = {
                "error": f"engine server not reachable at {host}:{port}"
            }
...
def _add_common_args(p, *, triton=False, engine=False, model=False, out=True):
    ...
    if triton:
        ...
        p.add_argument(
            "--timeout", type=float, default=60.0, help="Request timeout (s)"
        )
    if engine:
        ...
        p.add_argument(
            "--timeout", type=float, default=6000.0, help="Request timeout (s)"
        )
    if model:
        ...
        p.add_argument(
            "--greedy", action="store_true", help="Use greedy (do_sample=False)"
        )
        p.add_argument(
            "--device", default=None, help="Torch device (auto-detect if omitted)"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub = parser.add_subparsers(
        dest="mode", required=True, help="Comparison / generation mode"
    )
    ...
    p.add_argument(
        "--text", default="你好，这是一段用于听感对比的测试语音。", help="Input text"
    )
    ...
    _add_common_args(p, model=True)
    p.add_argument(
        "--text", default="你好，这是一段用于对比的测试语音。", help="Input text"
    )
    ...
    p.add_argument(
        "--text-file", default=None, help="Read text from file (overrides --text)"
    )
    p.add_argument(
        "--out-root",
        default=None,
        help="Output root dir (default: workspace/audio_compare/runs/)",
    )
    ...
    p.add_argument(
        "--triton-url", default="", help="Triton gRPC URL (empty=skip TRT path)"
    )
    ...
    p.add_argument(
        "--case", choices=["4a", "4b"], default="4a", help="Text case preset"
    )
    p.add_argument(
        "--text", default=None, help="Override text (instead of case preset)"
    )
    p.add_argument(
        "--official-modes",

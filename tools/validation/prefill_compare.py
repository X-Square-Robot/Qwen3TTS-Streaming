...
def _print_deprecation():
    print(
        "NOTE: official_prefill.py, compare_live_vs_exported_prefill.py, "
...
def build_prefill_like_official(
    ...
    *,
    ...
    if tc is None:
        raise RuntimeError("Model has no talker_config")
    ...
    if speaker and speaker.lower() in (getattr(tc, "spk_id", {}) or {}):
        spk_embed = talker.get_input_embeddings()(
            torch.tensor(
                tc.spk_id[speaker.lower()], device=device, dtype=input_id.dtype
            )
        )
    # language
    lang_id = None
    ...
    # special embeds
    bos_e, eos_e, pad_e = talker.text_projection(
    ...
    cpl = (
        ...
        else [[tc.codec_nothink_id, tc.codec_think_bos_id, tc.codec_think_eos_id]]
    )
    c0 = talker.get_input_embeddings()(
        torch.tensor(cpl, device=device, dtype=input_id.dtype)
    )
    c1 = talker.get_input_embeddings()(
        torch.tensor(
            [[tc.codec_pad_id, tc.codec_bos_id]], device=device, dtype=input_id.dtype
        )
    )
    codec_in = (
        torch.cat([c0, spk_embed.view(1, 1, -1), c1], dim=1)
        if spk_embed is not None
        else torch.cat([c0, c1], dim=1)
    )
    # role + pad+bos + codec
    role_e = talker.text_projection(talker.get_text_embeddings()(input_id[:, :3]))
    pre = (
        torch.cat((pad_e.expand(-1, codec_in.shape[1] - 2, -1), bos_e), dim=1)
        + codec_in[:, :-1]
    )
    ...
    if instruct_ids is not None:
        ...
        tie = torch.cat(
            (talker.text_projection(talker.get_text_embeddings()(instruct_ids)), tie),
            dim=1,
        )
    ...
    if non_streaming_mode:
        ...
        nt = tp.shape[1]
        te = torch.cat(
            (talker.text_projection(talker.get_text_embeddings()(tp)), eos_e), dim=1
        )
        cpe = talker.get_input_embeddings()(
            torch.full(
                (1, nt + 1), tc.codec_pad_id, device=device, dtype=input_id.dtype
            )
        )
        tie = torch.cat(
            [
    ...
    tth = (
        torch.cat(
            (talker.text_projection(talker.get_text_embeddings()(mid)), eos_e), dim=1
        )
        if mid.shape[1] > 0
        else eos_e
    )
...
def _run_live_vs_exported(args):
    ...
    talker = model.talker
    builder, weights, tok = _load_builder(
        model_dir, str(REPO_ROOT / "workspace/exported/custom-1.7b/weights"), device
    )
    raw_sp = torch.load(
    ...
    ids = torch.as_tensor(
        tok(OFFICIAL_ASSISTANT_FMT.format(text=args.text), return_tensors="pt")[
            "input_ids"
        ],
        device=device,
        dtype=torch.long,
    ...
    rt_sp = torch.cat(
        [weights.tts_bos_embed, weights.tts_eos_embed, weights.tts_pad_embed], dim=1
    )
    ex_sp = torch.cat(
        [
            raw_sp[k].to(device=device, dtype=torch.bfloat16)
            for k in ("tts_bos_embed", "tts_eos_embed", "tts_pad_embed")
        ],
        dim=1,
    )
    ...
    with torch.no_grad():
        o1 = talker.model(
            inputs_embeds=off_pf.to(device=device, dtype=torch.bfloat16),
            use_cache=True,
            return_dict=True,
        )
        o2 = talker.model(inputs_embeds=eng_pf, use_cache=True, return_dict=True)
    r1 = talker.codec_head(o1.last_hidden_state)[:, -1, :]
    r2 = talker.codec_head(o2.last_hidden_state)[:, -1, :]
    p1 = RepetitionPenaltyLogitsProcessor(args.repetition_penalty)(
        hist, r1.float().clone()
    )
    p2 = RepetitionPenaltyLogitsProcessor(args.repetition_penalty)(
        hist, r2.float().clone()
    )
    s = {
        "component_diffs": {
            ...
            "codec_prefill": tensor_diff(
                talker.get_input_embeddings()(c_ids), weights.codec_embed(c_ids)
            ),
        },
        "prefill_diff": tensor_diff(off_pf, eng_pf),
        "trailing_diffs": [tensor_diff(a, b) for a, b in zip(off_tr, eng_tr)],
        "forward": {
            "hidden_diff": tensor_diff(
                o1.last_hidden_state[:, -1:, :], o2.last_hidden_state[:, -1:, :]
            ),
            "logits_diff": tensor_diff(p1, p2),
            "official_t0": int(p1.argmax(dim=-1).item()),
            "engine_t0": int(p2.argmax(dim=-1).item()),
        },
    }
    out = Path(args.out_json)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(s, ensure_ascii=False, indent=2))
    for k, v in s["component_diffs"].items():
        print(f"  {k}:", json.dumps(v, ensure_ascii=False))
    print("  prefill_diff:", json.dumps(s["prefill_diff"], ensure_ascii=False))
    print(
        f"  token0: official={s['forward']['official_t0']} engine={s['forward']['engine_t0']}"
    )
    print("  saved:", out)
...
def _run_compare_paths(args):
    ...
    at = (
        OFFICIAL_ASSISTANT_FMT.format(text=args.text)
        if args.prompt_mode == "supported"
        else f"<|im_start|>assistant\n{args.text}<|im_end|>"
    )
    ids = torch.as_tensor(
        tok(at, return_tensors="pt")["input_ids"], device=device, dtype=torch.long
    )
    if ids.dim() == 1:
        ids = ids.unsqueeze(0)
    off_pf, off_tr = build_prefill_like_official(
        model, ids, args.language, args.speaker, device
    )
    text_ids = builder._encode_text_ids(args.text)
    plan = builder.build_plan_from_ids(
        task_type=TaskType.CUSTOM_VOICE,
    ...
    for i in range(min(len(off_tr), len(eng_tr))):
        print(
            f"trailing_diff[{i}]:",
            json.dumps(tensor_diff(off_tr[i], eng_tr[i]), ensure_ascii=False),
        )
    if len(off_tr) != len(eng_tr):
        print(f"trailing_len_mismatch: {len(off_tr)} vs {len(eng_tr)}")
...
def _sample_token(logits, hist, *, top_k, temperature, rep_pen, suppress):
    ...
    if suppress:
        s[..., suppress] = -1e9
    if rep_pen != 1.0:
        s = RepetitionPenaltyLogitsProcessor(rep_pen)(hist, s)
    if temperature != 1.0:
        s = TemperatureLogitsWarper(temperature)(hist, s)
    if top_k > 0:
        s = TopKLogitsWarper(top_k)(hist, s)
...
def _run_manual_rollout(args):
    ...
    with torch.no_grad():
        out = talker.model(inputs_embeds=pf, use_cache=True, return_dict=True)
    kv = out.past_key_values
    ph = out.last_hidden_state[:, -1:, :]
    logits = talker.codec_head(out.last_hidden_state)[:, -1, :]
    hist = torch.empty((1, 0), device=device, dtype=torch.long)
    c0 = _sample_token(
        logits, hist, top_k=50, temperature=0.9, rep_pen=1.05, suppress=suppress
    )
    ...
    for step in range(args.max_steps - 1):
        with torch.no_grad():
            cp_in = torch.cat(
                (ph, talker.model.codec_embedding(c0).unsqueeze(1)), dim=1
            )
            pred = cp.generate(
                inputs_embeds=cp_in,
            ...
            cpt = pred.sequences
            ch = torch.cat(
                [talker.get_input_embeddings()(c0.unsqueeze(1))]
                + [
            ...
            ni = ch.sum(1, keepdim=True) + (
                trailing[step] if step < len(trailing) else pad_e
            )
            so = talker.model(
                inputs_embeds=ni, past_key_values=kv, use_cache=True, return_dict=True
            )
        kv = so.past_key_values
        ph = so.last_hidden_state[:, -1:, :]
        ...
        hist = torch.cat([hist, c0.unsqueeze(1)], dim=1)
        seq.append(c0.item())
        phases.append("text" if step < len(trailing) else "pad")
        if c0.item() == talker.config.codec_eos_token_id:
            break
    # official rollout
    ...
    n = min(len(off), len(seq))
    div = next((i for i in range(n) if off[i] != seq[i]), -1)
    print(
        f"trailing_len: {len(trailing)}  official_len: {len(off)}  manual_len: {len(seq)}  first_div: {div}"
    )
    if div >= 0:
        ...
        for i in range(lo, hi):
            mk = "<<" if i == div else "  "
            print(
                f"{mk} step={i:03d} phase={phases[i] if i < len(phases) else 'n/a':7s} off={off[i]:4d} man={seq[i]:4d}"
            )
    else:
...
def _run_cp_parity(args):
    ...
    cp = talker.code_predictor
    cpu = (
        CodePredictorUnrolled(cp, talker.model.codec_embedding, logits_topk=args.top_k)
        .to(device)
        .eval()
    )
    builder, weights, tok = _load_builder(
    ...
    with torch.no_grad():
        out = talker.model(inputs_embeds=pf, use_cache=True, return_dict=True)
    ph = out.last_hidden_state[:, -1:, :]
    tl = talker.codec_head(out.last_hidden_state)[:, -1, :]
    c0 = tl.argmax(dim=-1)
    print(
        f"=== CP sampled parity ===  text={args.text}  codec0={c0.item()}  trailing={len(plan.trailing)}"
    )
    # greedy
    with torch.no_grad():
        og = (
        ...
        ug = (
            cpu(
                ph,
                c0,
                cp_gumbel_noise=None,
                temperature=torch.full(
                    (1, 1), args.temperature, device=device, dtype=torch.float32
                ),
            )[0]
            .cpu()
            .tolist()
    ...
    full = 0
    pfx = []
    oc = [Counter() for _ in range(15)]
    uc = [Counter() for _ in range(15)]
    for seed in range(args.trials):
        ...
        if o == u:
            full += 1
        lcp = next(
            (i for i, (a, b) in enumerate(zip(o, u)) if a != b), min(len(o), len(u))
        )
        pfx.append(lcp)
...
def main(argv=None) -> int:
    ...
    {
        "official": lambda: print(
            "Use --mode compare-paths or --mode manual-rollout for comparison."
        ),

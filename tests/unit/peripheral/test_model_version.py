...
def test_load_model_version_reads_package_root(tmp_path):
    ...
    assert load_model_version(tmp_path) == "researcher@20260601"
...
def test_write_model_version_creates_and_replaces_read_only_sidecar(tmp_path):
    ...
    assert write_version_sidecar(version_path, version="researcher@20260818") == (
        "researcher@20260818"
    )
    assert version_path.read_text(encoding="utf-8") == "researcher@20260818\n"
    assert version_path.stat().st_mode & 0o222 == 0

    write_version_sidecar(version_path, version="researcher@20260820")
    assert version_path.read_text(encoding="utf-8") == "researcher@20260820\n"
    assert version_path.stat().st_mode & 0o222 == 0
...
def test_write_version_sidecar_uses_field_specific_validation(tmp_path):
    with pytest.raises(ModelVersionError):
        write_version_sidecar(
            ...
            version="builder@20260902_580_5090_v1",
        )
    with pytest.raises(ModelVersionError):
        write_version_sidecar(
            tmp_path / ENGINE_BUILD_VERSION_FILENAME,
            version="researcher@20260818",
        )


def test_load_engine_build_version_uses_package_sidecar(tmp_path):
    version_path = tmp_path / ENGINE_BUILD_VERSION_FILENAME
    write_version_sidecar(version_path, version="builder@20260820_580_5090_v2")

    assert load_engine_build_version(tmp_path) == "builder@20260820_580_5090_v2"
...
def test_engine_build_version_derivation_does_not_include_model_release():
    artifact = {
        ...
        "gpu_sm": "sm_120",
        "export_protocol_version": "v1",
        "model_version": "researcher@20260818",
    }
    assert build_engine_build_version_from_artifact(artifact) == (
        "builder@20260902_580_5090_v1"
    )
    assert validate_engine_build_version("builder@20260902_580_5090_v1") == (
        "builder@20260902_580_5090_v1"
    )
...
@pytest.mark.parametrize(
    "value",
    [
        "builder@20261301_580_5090_v1",
        "builder@20260902_driver_5090_v1",
        "builder@20260902_580_5090_protocol1",
    ],
)

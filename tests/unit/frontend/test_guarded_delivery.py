...
class TestDeliveryHoldWindow:
    ...
    def test_discard_drops_held_without_crediting_release(self):
        ...
        assert win.held_bytes == 0
        # Later audio is still gated by the same playback clock.
        win.push([b"e" * 100])
        assert win.release_due() == []
        clock.now += 1.0
        assert [len(c) for c in win.release_due()] == [100]
...
class TestGuardedHoldPolicy:
    ...
    def test_disabled_by_default(self):
        assert FrontendInterface._guarded_hold_for(self._session({})) is None
        assert (
            FrontendInterface._guarded_hold_for(self._session({"delivery": "firehose"}))
            is None
        )
    ...
    def test_window_defaults_and_clamps(self):
        hold = FrontendInterface._guarded_hold_for(self._session({"delivery": "guarded"}))
        assert hold._window_sec == pytest.approx(1.5)
        hold = FrontendInterface._guarded_hold_for(
            self._session({"delivery": "GUARDED", "delivery_window_ms": "5"})
        )
...
def _fake_interface_self():
    ...
    return SimpleNamespace(
        _guarded_hold_for=FrontendInterface._guarded_hold_for,
...
def _run_consume(session, results, events=None):
    delivered = []
    done_meta = {}
...
class TestConsumeResultsGuarded:
    ...
    def test_firehose_default_passes_everything_through(self):
        session = _guarded_session()
        session.config.output_policy.config = {}
...
def _seg_end(seg_idx, eos_reason, audio_steps, tail=0):
    ...
    return EngineResult(
        type=ResultType.SEGMENT_END, session_id="s1", segment_idx=seg_idx,
        metrics=metrics,
    )
...
def _chunk(seg_idx, marker, frames=12):
    return EngineResult(
        type=ResultType.AUDIO_CHUNK, session_id="s1", segment_idx=seg_idx,

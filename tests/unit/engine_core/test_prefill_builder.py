...
class _TinyICLWeights:
    ...
    def codec_embed(self, codec_ids: torch.Tensor) -> torch.Tensor:
        return (
            (codec_ids.to(torch.float32) * 0.01)
            .unsqueeze(-1)
            .expand(
                *codec_ids.shape,
                self.hidden_size,
...
def test_embedding_weights_specials_match_runtime_projection():
    ...
    special_ids = torch.tensor(
        [
            [
                weights.tts_pad_token_id,
                weights.tts_bos_token_id,
                weights.tts_eos_token_id,
            ]
        ],
    ...
    torch.testing.assert_close(
        weights.tts_pad_embed, projected[:, 0:1, :], atol=0.0, rtol=0.0
    )
    torch.testing.assert_close(
        weights.tts_bos_embed, projected[:, 1:2, :], atol=0.0, rtol=0.0
    )
    torch.testing.assert_close(
        weights.tts_eos_embed, projected[:, 2:3, :], atol=0.0, rtol=0.0
    )
...
def test_prefill_plan_voice_design_exposes_cacheable_prefix():
    ...
    assert (
        plan.cacheable_prefix_embeds.shape[1] + plan.request_prefill_embeds.shape[1]
...
def test_parse_task_type():
    ...
    assert (
        parse_task_type("voice_clone", x_vector_only=True) == TaskType.VOICE_CLONE_XVEC
    )
    assert (
        parse_task_type("voice_clone", x_vector_only=False) == TaskType.VOICE_CLONE_ICL
    )
...
@pytest.mark.skip(
    reason="BF16 vs FP32 comparison requires legacy FP32 weights export; run manually if needed"
)
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required")
...
@pytest.mark.skip(
    reason="DLPack zero-copy is internal to model.py BLS; E2E run implies it works"
)

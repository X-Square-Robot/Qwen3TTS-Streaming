...
#    --model-release-version <id>  Research model MODEL_VERSION value
...
Options:
  ...
  --model-release-version <id>
                          Research model release stored in read-only MODEL_VERSION
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --engine-build-version)
                log_error "ENGINE_BUILD_VERSION is generated by Phase B; do not provide --engine-build-version"
                exit 2
                ;;
...
model_release, packager, package_date = sys.argv[1:]
try:
    if model_release:
...
_current_exported_model_release_version() {
    ...
    if [ -s "$version_path" ]; then
        PYTHONPATH="$REPO_ROOT" python3 - "$version_path" <<'PY' 2>/dev/null || true
import sys
...
try:
    ...
    # A legacy composite value must not be silently propagated as a model
    # release.  Let the TUI request a fresh researcher@YYYYMMDD value.
    pass
PY
    fi
}
...
build_forward_args() {
    ...
    append_optarg DEPLOY_ARGS --triton-model-version "$MODEL_VERSION"
    if $BUILD_IMAGE_EXPLICIT; then

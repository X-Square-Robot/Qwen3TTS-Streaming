...
docker run --rm --entrypoint python3 "$runtime_base_image" -c \
    "import aiohttp, attr, torch, tokenizers, tensorrt, yaml, grpc, numpy, soxr; print(f'torch={torch.__version__} cuda={torch.version.cuda} tensorrt={tensorrt.__version__} attrs={attr.__version__}')"

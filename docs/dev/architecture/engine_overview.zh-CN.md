...
| **C4** | 任意 checkpoint 上模型**可能不可靠吐 EOS**——频率**依赖 checkpoint**，且本项目不发布权重，所以这是引擎必须永久防御的常驻约束。历史内部 0601 权重在 ~10–18% 种子上永不收尾 → 跑满 512 → 幻觉；重训/当前 v0.2 checkpoint 在同一组确定性种子上 0/100（cp=fp32 与全 bf16 引擎均验证），因此当前发布线已大幅抑制该问题。512 步上限、VAD 裁剪、runaway 处理仍然是为用户自带任意 checkpoint 准备的 | 模型 checkpoint+采样，**非实现 bug** |
...
VAD 输出门控（裁前导/尾部异常静音）、等时音频流（补静音给 WebRTC jitter buffer）、14 阶段可观测性。
- **为什么**：v0.2 已从 checkpoint 层面修复主要跑飞源，但任意 checkpoint 与输入仍需要输出端防御（VAD ~0.1-0.2ms，对引擎透明）。
...
| 缺陷 | 根 | 能做的 |
|---|---|---|
| 历史 0601 跑飞型幻觉；v0.2 已验证 checkpoint 探针 0/100 | C4 模型+采样 | 重训/验证 checkpoint、VAD 善后、loop guard、EOS 调温、强制截断；用户自带 checkpoint 仍需验收 |

...
### 1.5 Sub-module Precision (CP fp32 historical; default now cp=bf16 + code2wav=fp16)
...
- **code2wav fp16 (default)**: TRT 10.13 on sm120 has no tensor-core bf16 conv kernels (fp16 convs are 2.5-3.4× faster), so the c2w vocoder costs ~35ms of a b128 decode step under bf16 vs ~9ms under fp16. `CODE2WAV_PRECISION` therefore now **defaults to fp16** in both `build_engines.sh` and `autorun.sh`. The emitter pins only `/code2wav/*` to fp16 and pins `/talker_fused/*` back to bf16 (so the global `--fp16` flag cannot let talker/CP kernels float and shift sampling numerics), and `build_engines.sh` auto-selects `--precisionConstraints=prefer` for the speed pin (`trt_fused_io_formats.py --emit constraints` — the c2w Pad/Slice glue has no fp16 kernel and must fall back). c2w does not feed back into the talker; validated via halluprobe 0/100 + audio level/spectrum checks. Unlike the CP, c2w fp16 is numerically fine. Set `CODE2WAV_PRECISION=bf16` for a uniform-bf16 build (numerical parity / baseline reproduction).

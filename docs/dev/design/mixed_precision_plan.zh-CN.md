...
>
> **2026-07-06 更新——cp=fp32 的动机已过时**：促成本方案的幻觉已定位为 0601 checkpoint 权重训坏,与 CP 精度无关（见 `../investigation/streaming_hallucination.zh-CN.md`）。0701 重训后,全 bf16 引擎（cp=bf16）在与 cp=fp32 基线相同的确定性探测集上实测 0/100,因此 `CP_PRECISION` 现默认跟随 `ENGINE_DTYPE`（bf16）。本方案建立的子模块精度控制*机制*保留且仍然有用，它也支撑 `CODE2WAV_PRECISION=fp16` **opt-in**（混合构建经 `trt_fused_io_formats.py --emit constraints` 自动选 `--precisionConstraints=prefer`）。但 fp16 c2w 只在低并发有利、高并发反而拖慢 decode step——交叉 ~c32，b128 **+8%**（实测 2026-07-08，见 `../architecture/engine_overview.zh-CN.md` §1.5），因为 c2w 流式状态每步都要做随 batch 放大的 bf16↔fp16 reformat。**因此默认仍是全 bf16**（cp=bf16、code2wav=bf16）。`--cp-precision fp32` 与 `--code2wav-precision fp16` 分别是数值对齐调试、单流/低并发延迟的 opt-in。
>

...
- **字段**：`segment_id` · `segment_text_preview`(钳) · `audio_steps` · `text_tokens` · `audio_text_ratio` ·
  `eos_reason`(codec_eos/silence_abort/kv_overflow/loop_abort) · `abort_tail_frames`(仅 abort 原因：确证损坏的尾帧数) · `overflow` · `cache_hit` · `segment_prefill_ms` · `batched`(bool) · `batch_size_at_prefill`

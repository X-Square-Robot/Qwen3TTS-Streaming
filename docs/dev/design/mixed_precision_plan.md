...
>
> **2026-07-06 update — the cp=fp32 motivation is obsolete**: the hallucination that motivated this plan was root-caused to the damaged 0601 checkpoint, not CP precision (see `../investigation/streaming_hallucination.md`). On the 0701 retrain, a full-bf16 engine (cp=bf16) measures 0/100 on the same deterministic probe set as the cp=fp32 baseline, so `CP_PRECISION` now defaults to follow `ENGINE_DTYPE` (bf16). The per-submodule precision *mechanism* built by this plan remains in place and useful. It also enables the `CODE2WAV_PRECISION=fp16` **opt-in** (the mixed build auto-selects `--precisionConstraints=prefer` via `trt_fused_io_formats.py --emit constraints`). But fp16 c2w only helps at low concurrency and regresses the decode step at high concurrency — crossover ~c32, b128 **+8%** (measured 2026-07-08, see `../architecture/engine_overview.md` §1.5) — because the c2w streaming states round-trip through a per-step bf16↔fp16 reformat that scales with batch. **The default therefore stays full-bf16** (cp=bf16, code2wav=bf16). `--cp-precision fp32` and `--code2wav-precision fp16` are opt-ins for numerical-parity debugging and single-stream/low-concurrency latency respectively.
>

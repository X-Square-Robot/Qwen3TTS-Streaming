...
**2026-07-07/08**,基于第三轮优化后的引擎构建(含 `b80cf45`/`2a9578b`/`a8e7274` 及
`cc58c6a`/`8596996`/`b80c38d`);`triton-grpc` 行**沿用 2026-07-06 数据集**,其引擎核心早于
上述全部 commit——这些数字高估了当前 Triton 延迟,仅作最近一次实测参考保留。
...
- `summary.csv` — 按 (协议, 并发档位) 和 (协议, 连接模式) 分组的 avg/p50/p90/p99/max,汇总**全部 6 轮**(engine 侧 3 轮 2026-07-07/08 + Triton 侧 3 轮 2026-07-06;共 51,981 条请求,0 失败)。
...
- **两侧的 `prefill_ms` 语义不同。** engine 行(2026-07-07/08):数值覆盖整个批量准入 pass(见报告"测试条件"的说明框),因此天然随并发增长。Triton 行(2026-07-06):旧的单 session 语义。负载下不要跨侧对比该列。

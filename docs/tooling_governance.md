...
- If a file here becomes a canonical validation workflow for users, it should usually move to `tests/tools/` and optionally leave behind a compatibility wrapper.
...
- Put reusable standalone-engine helpers here when both pytest files and `tests/tools/` need them.
- `tests/tools/` should not import `tests/e2e/test_*.py` directly when a support module can own that behavior.

### 7. Manual Validation Tools

- `tests/tools/`: acceptance, benchmark, audio inspection, and debug tools that are intentionally not pytest tests.
...
- `tests/repro/`: historical bug reproductions that should stay isolated from the normal validation surface.
...
2. If the behavior is exploratory, benchmark-oriented, or depends on human listening/inspection, put it in `tests/tools/`.
...
- `tests/tools/` must not use the `test_*.py` prefix.
...
- Compatibility wrappers should say so in their module docstring.

...
Each version tag creates a Release wheel on both forges. The authoritative,
copyable command is generated by the deployed instance at `/demo/#/sdk`; it
uses the exact relative `/sdk/` asset URL and therefore preserves reverse-proxy
prefixes. Forge users can alternatively select the wheel attached to the
...
Every release runtime image embeds the wheel already published by its tag

...
class TTSServicer(tts_pb2_grpc.TTSServiceServicer):
    ...
    async def _create_session(
        ...
        *,
        ...
        # Debug-only prefix/VAD timing probe.  Keep the wire contract unchanged
        # while exposing the first 64 raw chunks to diagnose synthesis-vs-VAD
        # gating (e.g. 500 ms prefixes under firehose/guarded delivery).
        vad_probe_count = 0
        ...
        async def on_audio(sid, data):
            ...
            if probe_enabled:
                logger.debug(
                    "VAD probe grpc session=%s idx=%d delivery=%s raw_ms=%.3f inter_ms=%.3f retained_ms=%.3f "
            ...
            if probe_enabled and queue_start:
                logger.debug(
                    "VAD probe grpc session=%s idx=%d queue_wait_ms=%.3f",
                    internal_session_id,
                    probe_idx,

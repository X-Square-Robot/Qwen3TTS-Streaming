...
**14.9 ± 0.3ms** server TTFT (n=50, min 14.5ms) is faster than a single 60Hz screen refresh (16.7ms) and well under the ~100–400ms a human eye takes to blink — the first audio chunk is already playing before a wait would even register. Under a 128-stream simultaneous burst the client-side average is 242–269ms depending on transport. Both numbers come with conditions attached; see [Performance Claims](#performance-claims) for exactly what they depend on.
...
- ⚡ **Token-level streaming, not sentence-level** — first audio chunk in ~15ms (server TTFT 14.9 ± 0.3ms), 242–269ms avg under a 128-stream burst

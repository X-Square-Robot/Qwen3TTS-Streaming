...
> ⚠️ **128 streams is the tested ceiling, not a safe production target.** After the 2026-07-06 decode optimizations (CP in-graph KV + CUDA-graph decode replay), the benchmarked GPU (RTX 5090, all-bf16 engine) sustains 128 concurrent streams at a decode step of ~70ms per ~80ms audio frame — RTF (audio duration / wall-clock decode time) ≈ 1.14, i.e. ~14% headroom above real-time (previously RTF ≈ 1 with none). That margin absorbs normal jitter, but a sustained load spike or heavier-than-usual requests can still eat it. Size production concurrency with margin below 128 rather than running at it; at 96 streams RTF ≈ 1.47 with ampler margin.
